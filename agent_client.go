...
func (c *AgentClient) CheckAvailability(ctx context.Context, req *livekit.CheckAvailabilityRequest) (*livekit.CheckAvailabilityResponse, error) {
	ctx, err := c.withAuth(ctx, withVideoGrant{RoomAdmin: true})
	if err != nil {
		return nil, err
	}

	return c.agentService.CheckAvailability(ctx, req)
}
